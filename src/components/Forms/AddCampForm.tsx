...
export default function AddCampForm({ lat, lng, onClose }: AddCampFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-zinc-900 border border-yellow-500/30 rounded-2xl p-8 w-full max-w-sm shadow-2xl relative shadow-yellow-500/10 transition-all text-[16px]">
        ...
        {successStatus ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <input required name="company" className="bg-zinc-950/50 border border-zinc-800 text-white rounded-xl p-4 outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-all placeholder:text-zinc-600 text-[16px]" placeholder={t('mining_company')} />
              <input required name="google_maps_link" type="url" className="bg-zinc-950/50 border border-zinc-800 text-white rounded-xl p-4 outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-all placeholder:text-zinc-600 text-[16px]" placeholder={t('google_maps_link')} />
              
              <button disabled={loading} type="submit" className="mt-4 bg-yellow-500 text-black font-black uppercase tracking-wider py-4 rounded-xl hover:bg-yellow-400 active:scale-95 transition-all w-full flex justify-center items-center text-[16px]">
                {loading ? (
